...
def setup_endpoints(host, admin_token, endpoints, public_host=None,
                    region=None):
    ...
    for service, data in endpoints.iteritems():
        ...
        conf.update(data)
        _register_endpoint(keystone, service, conf, public_host, region)
...
def _register_endpoint(keystone, service, data, public_host=None, region=None):
    ...
    if not data.get('nouser'):
        _create_user_for_service(keystone, service, data.get('password', None))

    LOG.debug('Creating service for %s.', data.get('type'))
    kservice = keystone.services.create(service, data.get('type'),
...
def _create_user_for_service(keystone, name, password):
    ...
    user = keystone.users.create(name, password, tenant_id=service_tenant.id,
