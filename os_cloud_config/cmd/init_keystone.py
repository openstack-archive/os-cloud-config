...
def parse_args(args):
    ...
    %prog -o <keystone-endpoint> -t <service-token> -e <admin-email>
        -p <admin-password> -u <user> [-r <region>] [-s <ssl-endpoint>]
...
def main():
    ...
    if not all((options.host, options.admin_token, options.admin_email,
               options.admin_password, options.user)):
        print("The --host, --admin-token, --admin-email, --admin-password "
              "and --user options are required.")
        return 1
