...
class RegisterNodesTest(base.TestCase):
    ...
    @mock.patch.object(sys, 'argv', ['register-nodes', '--nodes', 'foo'])
    def test_no_service_host(self, stdout_mock):
        register_nodes.main()
        self.assertEqual('A Service Host is required.\n',
                         stdout_mock.getvalue())

    @mock.patch('sys.stdout', new_callable=StringIO)
    @mock.patch.object(sys, 'argv', ['register-nodes', '--service-host',
                       'seed'])
    def test_no_nodes(self, stdout_mock):
        register_nodes.main()
