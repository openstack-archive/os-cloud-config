...
def _fake_driver_info(node):
    # The fake_pxe driver doesn't need any credentials since there's
    # no power management
    return None


def _associate_deploy_info(driver_info, node):
...
def _extract_driver_info(node):
    pxe_drivers = {"pxe_drac": _drac_driver_info,
                   "pxe_ssh": _ssh_driver_info,
                   "pxe_ilo": _ilo_driver_info,
                   "pxe_iboot": _iboot_driver_info,
                   "fake_pxe": _fake_driver_info}

    def _specify_driver_info(node):
        pm_type = node["pm_type"]
        if "ipmi" in pm_type:
            return _ipmi_driver_info(node)
        else:
            if pm_type in pxe_drivers:
                return pxe_drivers.get(pm_type)(node)
            else:
                raise ValueError("Unknown pm_type: %s" % node["pm_type"])

    driver_info = _specify_driver_info(node)
    driver_info = _associate_deploy_info(driver_info, node)
    return driver_info
