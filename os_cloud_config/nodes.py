...
def register_nova_bm_node(service_host, node):
    out = subprocess.check_output(["nova", "baremetal-node-create",
...
def register_ironic_node(node):
    out = subprocess.check_output(["ironic", "node-create", "-d",
                                  node["pm_type"]])
    ...
    if "impi" in node["pm_type"]:
        pm_password = node["pm_password"]
        impi_properties = ["driver_info/ipmi_address=%s" % node["pm_addr"],
                           "driver_info/ipmi_username=%s" % node["pm_user"],
                           "driver_info/impi_password=%s" % pm_password]
        node_properties.extend(impi_properties)
...
def register_all_nodes(service_host, nodes_list):
    for node in nodes_list:
        if service_host:
            register_nova_bm_node(service_host, node)
        else:
            register_ironic_node(node)
...
def check_service(service_host):
    if service_host:
