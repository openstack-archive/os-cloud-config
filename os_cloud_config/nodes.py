...
class DriverInfo(object):
    ...
    def unique_id_from_node(self, node, ports):
...
class iBootDriverInfo(PrefixedDriverInfo):
    ...
    def unique_ids_from_fields(self, fields):
        result = super(iBootDriverInfo, self).unique_ids_from_fields()[0]
        if 'iboot_relay_id' in fields:
            result = '%s#%s' % (result, fields['iboot_relay_id'])
        return [result]

    def unique_ids_from_node(self, node, ports):
        try:
            result = super(iBootDriverInfo, self).unique_ids_from_node()[0]
        except IndexError:
            return
