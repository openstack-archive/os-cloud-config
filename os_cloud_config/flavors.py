...
def create_flavors_from_nodes(client, node_list, kernel, ramdisk):
    ...
    for node in node_list:
        if os.environ.get('ROOT_DISK'):
            ephemeral = str(int(node['disk']) - int(os.environ['ROOT_DISK']))
            disk = os.environ['ROOT_DISK']
        else:
            ephemeral = None
            disk = node['disk']
